...
from m013_negative_binomial import MODEL_ID, MODEL_VERSION, walk_forward_predict
...
def build_canonical_inputs():
    ...
    if not audit_a["verified"] or not audit_b["verified"]:
        raise RuntimeError("M013_PINNED_2025_SOURCE_VERIFICATION_FAILED")
...
def build_outputs():
    ...
    if frozen.get("lineage") != "P002" or frozen.get("status") != "FROZEN_RESEARCH_RULE":
        raise RuntimeError("M013_P002_FROZEN_RULES_NOT_CONFIRMED")
    ...
    # The existing benchmark rows were already consumed by prior model work.
    # Therefore M013 cannot convert performance on them into independent evidence.
    model_state = "PAPER_ONLY"
    decision_weight = 0.0

    report = {
        "report_version": "M013_WALK_FORWARD_REPORT_V0_1",
        "model_id": MODEL_ID,
        ...
        "gate_results": {
            "development_diagnostic_vs_market": "PASS" if diagnostic_pass else "FAIL",
    ...
    if summary["development_market_evaluable_n"] > 0:
        benchmark_runtime = (
            f"M013 development-only date-batched walk-forward on {summary['development_market_evaluable_n']} previously consumed market-evaluable rows: "
            ...
            "Positive delta means challenger is descriptively better. This result is DEVELOPMENT_DIAGNOSTIC only."
        )
    else:
        benchmark_runtime = "No development market-evaluable rows were available."

    independent_runtime = (
        f"Independent validation rows available to M013 in this run={independent_n}; "
        f"frozen independent_validation_min_n={independent_min_n}. Previously consumed benchmark rows are not recounted as independent evidence."
    )

    evidence = {
        "artifact": "M013_Negative_Binomial_Total_Goals_v0.1",
        "artifact_sha256": None,
        "overall": {
            "TEST_AUTHORED": True,
            "TEST_EXECUTED": True,
            "TEST_PASS": bool(diagnostic_pass and independent_pass),
            "TEST_FAIL": not bool(diagnostic_pass and independent_pass),
            "explanation": (
                "Development benchmark and new independent validation both passed; downstream governance remains unchanged."
                if diagnostic_pass and independent_pass
                else "M013 remains PAPER_ONLY. Development benchmark performance cannot satisfy the independent-validation requirement, and no consumed row is promoted to new evidence."
            ),
        },
        "checks": [
            {
            ...
            },
            {
                ...
                "runtime_evidence": (
                    ...
                    "post_lineup_lambda is the prediction-time mean; dispersion is learned from strictly earlier verified total goals only; market fields are excluded from model fitting."
                ),
                "scope": "Gate2 dependency / anti-hindsight",
            },
            {
                "name": "development_diagnostic_vs_market",
                "status": "TEST_PASS" if diagnostic_pass else "TEST_FAIL",
                "executed": True,
                "runtime_evidence": benchmark_runtime,
                "scope": "Consumed benchmark diagnostic; not independent validation",
            },
            {
...
def main():
    ...
    parser.add_argument("--report", default=str(ROOT / "artifacts" / "m013-walkforward-report-v0.1.json"))
    parser.add_argument("--evidence", default=str(ROOT / "artifacts" / "m013" / "TEST_EVIDENCE.json"))
