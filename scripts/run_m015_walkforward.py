...
from m015_regularized_poisson_glm import MODEL_ID, MODEL_VERSION, walk_forward_predict
...
def build_canonical_inputs():
    ...
    if not audit_a["verified"] or not audit_b["verified"]:
        raise RuntimeError("M015_PINNED_2025_SOURCE_VERIFICATION_FAILED")
...
def build_outputs():
    ...
    if frozen.get("lineage") != "P002" or frozen.get("status") != "FROZEN_RESEARCH_RULE":
        raise RuntimeError("M015_P002_FROZEN_RULES_NOT_CONFIRMED")
    ...
    report = {
        "report_version": "M015_WALK_FORWARD_REPORT_V0_1",
        "model_id": MODEL_ID,
        ...
        "gate_results": {
            "development_diagnostic_vs_gate2": "PASS" if gate2_pass else "FAIL",
            "development_diagnostic_vs_market": "PASS" if market_pass else "FAIL",
    ...
    benchmark_runtime = (
        f"M015 development-only date-batched walk-forward on {summary['development_market_evaluable_n']} previously consumed market-evaluable rows: "
        ...
        "Positive delta means M015 is descriptively better. These rows remain DEVELOPMENT_DIAGNOSTIC only."
    ) if summary["development_market_evaluable_n"] > 0 else "No development market-evaluable rows were available."

    independent_runtime = (
        f"Independent validation rows available to M015 in this run={independent_n}; "
        f"frozen independent_validation_min_n={independent_min_n}. Previously consumed rows are not recounted as independent evidence."
    )

    evidence = {
        "artifact": "M015_Regularized_Poisson_GLM_Gate2_Components_v0.1",
        "artifact_sha256": None,
        "overall": {
            "TEST_AUTHORED": True,
            "TEST_EXECUTED": True,
            "TEST_PASS": bool(gate2_pass and market_pass and independent_pass),
            "TEST_FAIL": not bool(gate2_pass and market_pass and independent_pass),
            "explanation": (
                "Development comparisons and new independent validation all passed; downstream governance remains unchanged."
                if gate2_pass and market_pass and independent_pass
                else "M015 remains PAPER_ONLY. Development evidence cannot satisfy independent validation, and any failed descriptive comparison is preserved without retuning."
            ),
        },
        "checks": [
            {
            ...
            },
            {
                ...
                "runtime_evidence": (
                    ...
                    "GLM uses only Gate2 venue/last10/last5 component lambdas at prediction time; verified total goals train only strictly later date batches; market fields are excluded from fitting."
                ),
                "scope": "Gate2 dependency / anti-hindsight",
            },
            {
                "name": "development_diagnostic_vs_gate2",
                "status": "TEST_PASS" if gate2_pass else "TEST_FAIL",
                "executed": True,
                "runtime_evidence": benchmark_runtime,
                "scope": "Consumed development benchmark vs canonical Gate2",
            },
            {
                "name": "development_diagnostic_vs_market",
                "status": "TEST_PASS" if market_pass else "TEST_FAIL",
                "executed": True,
                "runtime_evidence": benchmark_runtime,
                "scope": "Consumed development benchmark vs market",
            },
            {
...
def main():
    ...
    parser.add_argument("--report", default=str(ROOT / "artifacts" / "m015-walkforward-report-v0.1.json"))
    parser.add_argument("--evidence", default=str(ROOT / "artifacts" / "m015" / "TEST_EVIDENCE.json"))
