...
export function evaluateAccess(req, permissions) {
    const reasons = [];
    if (!req.identity.active)
        reasons.push("IDENTITY_INACTIVE");
    if (req.identity.expires_at && Date.parse(req.identity.expires_at) <= Date.parse(req.requested_at))
        reasons.push("IDENTITY_EXPIRED");
    if (!req.identity.environment_scope.includes(req.environment))
        reasons.push("ENVIRONMENT_OUT_OF_SCOPE");
    const matched = permissions.filter(p => req.identity.roles.includes(p.role) &&
    ...
    if (!matched.length)
        reasons.push("NO_MATCHING_PERMISSION");
    ...
    if (req.break_glass) {
        if (req.environment !== "PRODUCTION")
            reasons.push("BREAK_GLASS_PRODUCTION_ONLY");
        if (!req.reason.trim())
            reasons.push("BREAK_GLASS_REASON_REQUIRED");
    }
    return {
