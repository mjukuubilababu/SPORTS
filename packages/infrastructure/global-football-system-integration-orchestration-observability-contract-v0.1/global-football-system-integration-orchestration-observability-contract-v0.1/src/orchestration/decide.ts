import type{CircuitBreakerState,EventStatus,OrchestrationDecision,PipelineStage,StageRun,SystemEvent}from"../domain/types.js";
...
export function decideOrchestration(
  ...
  if(circuit?.state==="OPEN") return {action:"BLOCKED_BY_CIRCUIT",reason:"stage circuit breaker open",next_retry_at:null};
  const d=depsFor(targetStage);
  const missing=d.requires.filter(x=>!completedStages.has(x));
  if(missing.length) return {action:"WAIT",reason:`missing dependencies: ${missing.join(",")}`,next_retry_at:null};

  const same=priorRuns.filter(r=>r.stage===targetStage&&r.input_hash===hashEvent(event));
  if(same.some(r=>r.status==="SUCCEEDED")) return {action:"SKIP_IDEMPOTENT",reason:"identical input already succeeded",next_retry_at:null};

  const failures=same.filter(r=>r.status==="FAILED").length;
  if(failures>=d.dead_letter_after_attempts) return {action:"DEAD_LETTER",reason:"retry budget exhausted",next_retry_at:null};
...
export function hashEvent(e:SystemEvent):string{
