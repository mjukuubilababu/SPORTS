...
export function rankFullMarketCandidates({reasoning,distribution,selections,observations,teamAState,teamBState,modelConfidence=1,worlds,contradictions}){
 ...
 const rows=selections.map(selection=>{
  const withinSupport=componentWithinDistributionSupport(distribution,selection);const mapped=mapReasoningToMarketSelection(reasoning,selection);const fallbackProbability=withinSupport?distributionProbability(distribution,selection):null;const modelProbability=withinSupport?(mapped.status==='MODELLED'?mapped.modelProbability:fallbackProbability):null;const obs=obsByKey.get(candidateKey(selection));
