...
function componentWithinDistributionSupport(distribution,component){const maxHome=Math.max(...distribution.rows.map(row=>row.homeGoals)),maxAway=Math.max(...distribution.rows.map(row=>row.awayGoals)),maxFullyCoveredTotal=Math.min(maxHome,maxAway);if(component.marketFamily==='CORRECT_SCORE_FULL_TIME'){const [home,away]=String(component.selection).split('-').map(Number);return home<=maxHome&&away<=maxAway;}if(component.marketFamily==='TOTAL_GOALS_EXACT_FULL_TIME')return Number(component.selection)<=maxFullyCoveredTotal;if(component.marketFamily==='MULTIGOALS_FULL_TIME'){const bounds=String(component.selection??'').match(/^(\d+)-(\d+)$/);const max=component.maxGoals??(bounds?Number(bounds[2]):null);return Number.isInteger(max)&&max<=maxFullyCoveredTotal;}if(component.marketFamily==='TOTAL_GOALS_OVER_UNDER_FULL_TIME')return Number.isFinite(component.line)&&component.line<maxFullyCoveredTotal;if(component.marketFamily==='HOME_TEAM_OVER_UNDER_FULL_TIME')return Number.isFinite(component.line)&&component.line<maxHome;if(component.marketFamily==='AWAY_TEAM_OVER_UNDER_FULL_TIME')return Number.isFinite(component.line)&&component.line<maxAway;return true;}
function distributionProbability(distribution,component){if(!componentWithinDistributionSupport(distribution,component))return null;const probe=selectionOutcome(component,distribution.rows[0]);if(probe==='UNSUPPORTED')return null;const win=distributionMass(distribution,component,'WIN'),loss=distributionMass(distribution,component,'LOSS'),push=distributionMass(distribution,component,'PUSH');if(push>0&&win+loss<=0)return null;return push>0?win/(win+loss):win;}
...
const COMPLETE_MARKET_OUTCOMES=Object.freeze({'1X2_FULL_TIME':['HOME','DRAW','AWAY'],'DRAW_NO_BET_FULL_TIME':['HOME','AWAY'],'TOTAL_GOALS_OVER_UNDER_FULL_TIME':['OVER','UNDER'],'HOME_TEAM_OVER_UNDER_FULL_TIME':['OVER','UNDER'],'AWAY_TEAM_OVER_UNDER_FULL_TIME':['OVER','UNDER'],'BTTS_FULL_TIME':['YES','NO'],'CLEAN_SHEET_HOME':['YES','NO'],'CLEAN_SHEET_AWAY':['YES','NO'],'ODD_EVEN_FULL_TIME':['ODD','EVEN'],'TEAM_TO_SCORE_4WAY_FULL_TIME':['BOTH','HOME_ONLY','AWAY_ONLY','NEITHER']});
function hasCompleteOutcomeCoverage(rows){if(!rows.length)return false;const family=rows[0].marketFamily;if(rows.some(row=>row.marketFamily!==family))return false;const required=COMPLETE_MARKET_OUTCOMES[family];if(!required)return false;const actual=new Set(rows.map(row=>row.selection));if(rows.length!==required.length||actual.size!==required.length||!required.every(selection=>actual.has(selection)))return false;if(family.includes('OVER_UNDER'))return new Set(rows.map(row=>row.line)).size===1;return true;}
...
export function devigMarketObservations(observations,frozenAt,kickoffAt){
 ...
 for(const rows of groups.values()){if(rows.every(x=>x.completeMarket===true&&x.market_raw_probability!==null&&x.provenance_ready===true)&&hasCompleteOutcomeCoverage(rows)){const z=rows.reduce((s,x)=>s+x.market_raw_probability,0);for(const row of rows)row.market_fair_probability=row.market_raw_probability/z;}}
...
export function rankFullMarketCandidates({reasoning,distribution,selections,observations,teamAState,teamBState,modelConfidence=1,worlds,contradictions}){
 if(!finite01(modelConfidence))throw new Error('MODEL_CONFIDENCE_INVALID');
 ...
 const rows=selections.map(selection=>{
  const withinSupport=componentWithinDistributionSupport(distribution,selection);const mapped=mapReasoningToMarketSelection(reasoning,selection);const fallbackProbability=withinSupport?distributionProbability(distribution,selection):null;const modelProbability=withinSupport&&Number.isFinite(fallbackProbability)?(mapped.status==='MODELLED'?mapped.modelProbability:fallbackProbability):null;const obs=obsByKey.get(candidateKey(selection));
