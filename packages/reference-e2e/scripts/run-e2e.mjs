...
const result=runVerticalSlice({...fixture,store,audit,clock});
...
console.log(JSON.stringify({E2E:'PASS',state:result.state,assurance:result.assurance.gate,decision:result.decision.decision,paperExecution:result.execution.id,settlementWon:result.settlement.won,brier:result.evaluation.brier,logLoss:result.evaluation.logLoss,clv:result.evaluation.clv,auditEvents:result.audit.length,traceStages:result.trace.map(x=>x.stage)},null,2));
