...
import {createCounterfactualReplay,evaluateBatch} from '../src/observation.mjs';import {persistBatch,persistReplay} from '../src/postgres-persistence.mjs';
...
test('PostgreSQL 16 enforces immutable, atomic, idempotent observation evidence',{skip:!url},async()=>{const pool=new pg.Pool({connectionString:url,max:4});
 try{
  ...
  const replayInput={replay_id:'exact-replay',event_id:'event-a',cutoff_at:'2026-01-01T11:00:00Z',decision:'ABSTAIN',probabilities:{home:.4,draw:.3,away:.3},inputs:{odds:[2,3,4]},created_at:'2026-01-02T00:00:00Z'};
  const replay=createCounterfactualReplay(timed.observations[0],replayInput);await persistReplay(pool,replay);await persistReplay(pool,replay);
  await assert.rejects(()=>persistReplay(pool,createCounterfactualReplay(timed.observations[0],{...replayInput,decision:'HOME'})),/CHANGED_REPLAY/);
