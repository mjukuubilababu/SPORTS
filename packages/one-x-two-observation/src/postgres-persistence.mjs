...
export async function persistBatch(db,evaluation,{failAfter=null}={}){
 return inTransaction(db,async client=>{
  const first=evaluation.observations[0];const members=evaluation.observations.map(o=>({observation_id:o.observation_id,payload_fingerprint:o.payload_fingerprint})).sort((a,b)=>a.observation_id.localeCompare(b.observation_id));const batchFingerprint=fingerprint({batch_id:evaluation.batch_id,dataset_type:evaluation.dataset_type,status:evaluation.status,n:evaluation.n,wins:evaluation.wins,losses:evaluation.losses,hit_rate:evaluation.hit_rate,members});
