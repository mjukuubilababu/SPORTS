...
CREATE TABLE IF NOT EXISTS prediction_validations_v01(
  ...
  real_money text NOT NULL CHECK(real_money='NO'),
  FOREIGN KEY(prediction_snapshot_id,event_id) REFERENCES prediction_snapshots_v01(snapshot_id,event_id),
  FOREIGN KEY(outcome_id,outcome_fingerprint,event_id) REFERENCES prediction_outcomes_v01(outcome_id,outcome_fingerprint,event_id),
...
BEGIN
  SELECT s.kickoff_at INTO lineage_kickoff
   ...
   WHERE l.prediction_snapshot_id=NEW.prediction_snapshot_id AND l.event_id=NEW.event_id;
...
BEGIN
  SELECT observed_at INTO outcome_observed FROM prediction_outcomes_v01
   WHERE outcome_id=NEW.outcome_id AND outcome_fingerprint=NEW.outcome_fingerprint
     AND event_id=NEW.event_id AND prediction_snapshot_id=NEW.prediction_snapshot_id;
