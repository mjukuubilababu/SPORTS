  ...
  return Object.freeze({
    ...
    async attestPredictionLineage({snapshotId}){
      ...
      try{
        ...
        const modelSemantics=first.snapshot_type!=='PREMATCH'||(Array.isArray(consumedModels)&&consumedModels.length===1&&Array.isArray(reportedModels)&&reportedModels.length===1&&consumedModels[0]?.modelVersion===first.model_version&&consumedModels[0]?.snapshotId===first.model_snapshot_id&&consumedModels[0]?.snapshotSha256===first.model_fingerprint&&reportedModels[0]?.modelVersion===first.model_version&&reportedModels[0]?.snapshotId===first.model_snapshot_id&&reportedModels[0]?.snapshotSha256===first.model_fingerprint);
