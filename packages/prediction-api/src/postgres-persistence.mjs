  ...
  return Object.freeze({
    ...
    async attestPredictionLineage({snapshotId}){
      ...
      try{
        ...
        try{const expectedOutput=deterministicPredictionOutput(first.endpoint,first.input_payload);deterministicOutput=sha256Json(expectedOutput)===first.output_sha256&&String(expectedOutput.market??'1X2')===first.market&&(expectedOutput.selection==null?null:String(expectedOutput.selection))===first.selection;projectionMetadata=first.snapshot_type!=='LIVE'||(first.prediction_feature_version===expectedOutput.audit.featureVersion&&first.prediction_source_observed_at!==null&&dbIso(first.prediction_source_observed_at)===dbIso(expectedOutput.audit.observedAt));}catch{}
