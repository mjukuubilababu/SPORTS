...
export function canonicalModelIdentity(value){const scalar=canonicalScalarIdentity(value);return typeof scalar==='string'?scalar.trim():null;}
export function canonicalTimestampEpoch(value){const epoch=Date.parse(value);return Number.isFinite(epoch)?epoch:null;}
export function canonicalInputTimestamp(value){const epoch=canonicalTimestampEpoch(value);return epoch===null?null:new Date(epoch).toISOString();}
...
export function deterministicPredictionOutput(endpoint,input){
  ...
  if(!input||typeof input!=='object'||Array.isArray(input)||!input.preMatchSnapshot||typeof input.preMatchSnapshot!=='object'||Array.isArray(input.preMatchSnapshot)||!input.live||typeof input.live!=='object'||Array.isArray(input.live)||!input.live.eventId||input.live.eventId!==input.preMatchSnapshot.eventId||!Array.isArray(input.live.evidence)||input.live.evidence.length===0||!input.live.evidence.some(item=>item&&typeof item==='object'&&item.verified===true))throw new Error('LIVE_INPUT_NOT_CANONICAL');
...
function assertPersistenceInput({requestId,endpoint,input,output}){
  ...
  if(endpoint==='/v1/predict/live'){
    ...
    if(createdAt===null||frozenAt===null||canonicalTimestampEpoch(input.preMatchSnapshot.createdAt)>canonicalTimestampEpoch(input.preMatchSnapshot.frozenAt))throw persistenceError('PERSISTENCE_LIVE_SNAPSHOT_TIMESTAMPS_INVALID',409);
  }
  ...
  return Object.freeze({
    ...
    async attestPredictionLineage({snapshotId}){
      ...
      try{
        ...
        const canonicalEvent=canonicalScalarIdentity;
        ...
        const reportedModelsMatch=first.prediction_payload?.state==='WAIT'?Array.isArray(reportedModels)&&reportedModels.length===0:Array.isArray(reportedModels)&&reportedModels.length===1&&canonicalModelIdentity(reportedModels[0]?.modelVersion)===first.model_version&&canonicalModelIdentity(reportedModels[0]?.snapshotId)===first.model_snapshot_id&&reportedModels[0]?.snapshotSha256===first.model_fingerprint;
        const consumedModelPayload=consumedModel&&Object.fromEntries(Object.entries(consumedModel).filter(([key])=>key!=='snapshotSha256'));
        const modelSemantics=first.snapshot_type==='PREMATCH'
          ?Array.isArray(consumedModels)&&consumedModels.length===1&&reportedModelsMatch&&canonicalModelIdentity(consumedModels[0]?.modelVersion)===first.model_version&&canonicalModelIdentity(consumedModels[0]?.snapshotId)===first.model_snapshot_id&&canonicalInputTimestamp(consumedModels[0]?.frozenAt)===dbIso(first.model_frozen_at)&&consumedModels[0]?.snapshotSha256===first.model_fingerprint&&sha256ReferencePayload(consumedModelPayload)===first.model_payload_fingerprint
          :first.input_payload?.preMatchSnapshot?.signalId===first.frozen_signal_snapshot_id&&canonicalModelIdentity(first.input_payload?.preMatchSnapshot?.modelVersion)===first.model_version&&result.rows.some(row=>canonicalScalarIdentity(first.input_payload?.preMatchSnapshot?.featureVersion)===row.feature_version)&&canonicalInputTimestamp(first.input_payload?.preMatchSnapshot?.createdAt)!==null&&canonicalTimestampEpoch(first.input_payload?.preMatchSnapshot?.createdAt)<=canonicalTimestampEpoch(first.input_payload?.preMatchSnapshot?.frozenAt)&&canonicalInputTimestamp(first.input_payload?.preMatchSnapshot?.frozenAt)===dbIso(first.signal_frozen_at)&&canonicalModelIdentity(first.prediction_model_version)===first.model_version&&canonicalModelIdentity(first.prediction_payload?.audit?.modelVersion)===first.model_version&&sha256ReferencePayload(first.input_payload?.preMatchSnapshot)===first.signal_payload_fingerprint;
        if(!completeFeatures||!governed||!prematch||!hashes||!temporal||!endpointType||!liveParent||!predictionSemantics||!requestKickoff||!deterministicOutput||!projectionMetadata||!modelSemantics||!['FROZEN_SIGNAL','FROZEN_PREDICTION'].includes(first.signal_kind))throw persistenceError('PREDICTION_LINEAGE_ATTESTATION_FAILED',409);
