  ...
  return Object.freeze({
    ...
    async attestPredictionLineage({snapshotId}){
      ...
      try{
        ...
        const reportedModelsMatch=first.prediction_payload?.state==='WAIT'?Array.isArray(reportedModels)&&reportedModels.length===0:Array.isArray(reportedModels)&&reportedModels.length===1&&canonicalScalarIdentity(reportedModels[0]?.modelVersion)===first.model_version&&canonicalScalarIdentity(reportedModels[0]?.snapshotId)===first.model_snapshot_id&&reportedModels[0]?.snapshotSha256===first.model_fingerprint;
        ...
        const modelSemantics=first.snapshot_type==='PREMATCH'
          ?Array.isArray(consumedModels)&&consumedModels.length===1&&reportedModelsMatch&&canonicalScalarIdentity(consumedModels[0]?.modelVersion)===first.model_version&&canonicalScalarIdentity(consumedModels[0]?.snapshotId)===first.model_snapshot_id&&consumedModels[0]?.snapshotSha256===first.model_fingerprint&&sha256ReferencePayload(consumedModelPayload)===first.model_payload_fingerprint
          :first.input_payload?.preMatchSnapshot?.signalId===first.frozen_signal_snapshot_id&&canonicalScalarIdentity(first.input_payload?.preMatchSnapshot?.modelVersion)===first.model_version&&first.prediction_model_version===first.model_version&&canonicalScalarIdentity(first.prediction_payload?.audit?.modelVersion)===first.model_version&&sha256ReferencePayload(first.input_payload?.preMatchSnapshot)===first.signal_payload_fingerprint;
