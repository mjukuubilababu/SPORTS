  ...
  return Object.freeze({
    ...
    async attestPredictionLineage({snapshotId}){
      ...
      try{
        ...
        const prematch=result.rows.every(row=>row.pre_match_eligible===true&&row.is_verified===true&&row.prediction_cutoff!==null&&new Date(row.observed_at)<=new Date(row.available_at)&&new Date(row.available_at)<=new Date(row.source_captured_at)&&new Date(row.available_at)<=new Date(row.prediction_cutoff)&&new Date(row.source_captured_at)<=new Date(row.prediction_cutoff)&&new Date(row.source_captured_at)<=new Date(row.model_frozen_at)&&!['SETTLEMENT','PREDICTION_SETTLEMENT'].includes(row.evidence_kind));
        ...
        const modelSemantics=first.snapshot_type==='PREMATCH'
          ?Array.isArray(consumedModels)&&consumedModels.length===1&&Array.isArray(reportedModels)&&reportedModels.length===1&&consumedModels[0]?.modelVersion===first.model_version&&consumedModels[0]?.snapshotId===first.model_snapshot_id&&consumedModels[0]?.snapshotSha256===first.model_fingerprint&&reportedModels[0]?.modelVersion===first.model_version&&reportedModels[0]?.snapshotId===first.model_snapshot_id&&reportedModels[0]?.snapshotSha256===first.model_fingerprint&&sha256ReferencePayload(consumedModelPayload)===first.model_payload_fingerprint
        ...
        if(!completeFeatures||!governed||!prematch||!hashes||!temporal||!endpointType||!liveParent||!predictionSemantics||!deterministicOutput||!modelSemantics||!['FROZEN_SIGNAL','FROZEN_PREDICTION'].includes(first.signal_kind))throw persistenceError('PREDICTION_LINEAGE_ATTESTATION_FAILED',409);
