  ...
  return Object.freeze({
    ...
    async attestPredictionLineage({snapshotId}){
      ...
      try{
        ...
        const prematch=result.rows.every(row=>row.pre_match_eligible===true&&row.is_verified===true&&row.prediction_cutoff!==null&&new Date(row.observed_at)<=new Date(row.available_at)&&new Date(row.available_at)<=new Date(row.source_captured_at)&&new Date(row.available_at)<=new Date(row.prediction_cutoff)&&new Date(row.source_captured_at)<=new Date(row.prediction_cutoff)&&new Date(row.source_captured_at)<=new Date(row.model_frozen_at)&&new Date(row.feature_created_at)<=new Date(row.model_frozen_at)&&!['SETTLEMENT','PREDICTION_SETTLEMENT'].includes(row.evidence_kind));
