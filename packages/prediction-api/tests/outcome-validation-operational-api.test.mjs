...
test('health fails closed when outcome validation storage is unavailable',async()=>{
  ...
  const pool={async connect(){throw new Error('unused');},async query(){return {rowCount:1,rows:[{outcomes_table:'prediction_outcomes_v01',validations_table:'prediction_validations_v01',outcomes_select:true,outcomes_insert:true,validations_select:true,validations_insert:true,upstream_select:true}]};}};
  ...
  const deniedPool={async connect(){throw new Error('unused');},async query(){return {rowCount:1,rows:[{outcomes_table:'prediction_outcomes_v01',validations_table:'prediction_validations_v01',outcomes_select:true,outcomes_insert:false,validations_select:true,validations_insert:true}]};}};
  ...
  const upstreamDeniedPool={async connect(){throw new Error('unused');},async query(){return {rowCount:1,rows:[{outcomes_table:'prediction_outcomes_v01',validations_table:'prediction_validations_v01',outcomes_select:true,outcomes_insert:true,validations_select:true,validations_insert:true,upstream_select:false}]};}};
  await assert.rejects(createPredictionOutcomeValidationPersistence(upstreamDeniedPool,{attestPredictionLineage:async()=>null}).healthCheck(),/POSTGRES_OUTCOME_VALIDATION_UNAVAILABLE/);
});
